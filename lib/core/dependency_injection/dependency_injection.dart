...
import 'package:podcast_app/features/episode_details/data/datasources/remote/remote_episode_source_impl.dart';
...
import 'package:podcast_app/features/podcast_details/data/datasources/remote/remote_podcast_source_impl.dart';
...
import '../../features/podcast_details/data/datasources/remote/remote_podcast_source.dart';
...
void setupLocator() {
  ...
  // Data Sources
  getIt.registerLazySingleton<RemotePodcastSource>(
    () => RemotePodcastSourceImpl(),
  );

  getIt.registerLazySingleton<RemoteEpisodeSource>(
    () => RemoteEpisodeSourceImpl(),
  );

  // Repositories
  getIt.registerLazySingleton<PodcastRepository>(() => PodcastRepositoryImpl(
        remotePodcastSource: getIt(),
      ));

  getIt.registerLazySingleton<EpisodeRepository>(() => EpisodeRepositoryImpl(
        remoteEpisodeSource: getIt(),
      ));

  // Use Cases
  getIt.registerLazySingleton(() => FetchPodcastsUsecase(
        getIt(),
      ));

  getIt.registerLazySingleton(() => FetchEpisodesUsecase(
        getIt(),
      ));
}
